        ...
        if (strValue == "") {
              ...
              if (config.multiLanguageSupport) {
                ...
                errors.push(
                  capitalizeFirstLetter(key) +
                    " is not a valid!! Space , special chars not supported"
                );
              }
            }
          }
          if (rule.max !== undefined && rule.max > 0) {
            if (strValue.length > rule.max) {
              if (config.multiLanguageSupport) {
                errors.push(
                  key.toString() + "_must_be_lower_than_" + rule.max + "_chars"
                );
              } else {
                errors.push(
                  capitalizeFirstLetter(key) +
      ...
      if (rule.type == validate.number && rule.required) {
        if (isNaN(numValue)) {
          if (config.multiLanguageSupport) {
            ...
            errors.push(
              capitalizeFirstLetter(key) + " is not a valid number  "
            );
          }
        }
      }
        ...
        if (numValue == 0) {
          ...
          if (!Number.isInteger(newVal)) {
            if (config.multiLanguageSupport) {
              ...
              errors.push(
                capitalizeFirstLetter(key) + " is not a valid integer"
              );
            }
          }
        }
