...
import {Chef,ChefQuery,ChefKey, ChefService } from '@services/Chef';
...
expertise: { operator: FilterOperator.AND, constraints: [{ value: null, matchMode: FilterMatchMode.IN }] },
...
kitchen: { operator: FilterOperator.AND, constraints: [{ value: null, matchMode: FilterMatchMode.CONTAINS }] }
    ...
    const showUploadDialog = (chef:Chef,dbColName:string,accept:string="images/*") => {
        setChef({ ...chef });
...
return <Calendar value={options.value} onChange={(e) => options.filterCallback(e.value, options.index)} showTime hourFormat="12" />;
    ...
    return (
            <>
                ...
                <MultiSelect value={options.value} options={datamealItems}  onChange={(e) => options.filterCallback(e.value)} optionLabel="name" optionValue="name" placeholder="Any" className="p-column-filter" />
            </>
        );
    }
        ...
        if (chef.id) {
            ...
            let d=  await chefService.updateChef(_chef);
                if(d.error==undefined){
                    ...
                    toast.current?.show({ severity: 'success', summary: 'Loaded', detail: 'Chef Updated', life: 3000 });
                }else{
...
const deleteChef = async() => {
    ...
    if(d.error==undefined){
        ...
        setDeleteChefDialog(false);
        setChef(emptyChef);
        toast.current?.show({ severity: 'warn', summary: 'Deleted', detail: 'Chef Deleted', life: 3000 });
    }else{
...
const deleteSelectedChefs = () => {
    ...
    setDeleteChefsDialog(false);
    setSelectedChefs([]);
    toast.current?.show({ severity: 'success', summary: 'Successful', detail: 'Chefs Deleted', life: 3000 });
};
...
const localFilter=(val:string)=>{
    ...
    if(val.length>1){
        let _chefs =[...chefs];
        let filtered = _chefs.filter((data) =>  JSON.stringify(data).toLowerCase().indexOf(val.toLowerCase()) !== -1);
        setChefs(filtered);
    ...
    return (
        <React.Fragment>
            <div className="my-2">
                ...
                <Button label="Delete" icon="pi pi-trash" className="p-button-danger" onClick={confirmDeleteSelected} disabled={!selectedChefs || !selectedChefs.length} />
            </div>
        </React.Fragment>
    );
};
...
const actionBodyTemplate = (rowData:Chef) => {
    return (
        <>
            <Button icon="pi pi-pencil" className="p-button-rounded p-button-success mr-2" onClick={() => editChef(rowData)} />
            <Button icon="pi pi-trash" className="p-button-rounded p-button-warning" onClick={() => confirmDeleteChef(rowData)} />
...
const chefDialogFooter = (
    <>
        ...
        <Button label="Save" icon="pi pi-check" className="p-button-text" onClick={saveChef} />
    </>
);
const deleteChefDialogFooter = (
    <>
        <Button label="No" icon="pi pi-times" className="p-button-text" onClick={hideDeleteChefDialog} />
        <Button label="Yes" icon="pi pi-check" className="p-button-text" onClick={deleteChef} />
    </>
);
const deleteChefsDialogFooter = (
    <>
        <Button label="No" icon="pi pi-times" className="p-button-text" onClick={hideDeleteChefsDialog} />
        <Button label="Yes" icon="pi pi-check" className="p-button-text" onClick={deleteSelectedChefs} />
    </>
);
...
return (
    ...
    <Column showAddButton={false}  field="expertise" header="Expertise" sortable  headerStyle={{ minWidth: '10rem' }} filterField="expertise"   filter filterElement={expertiseFilterTemplate}  ></Column>
            

    <Column showAddButton={false}  field="phoneNumber" header="phoneNumber" sortable  headerStyle={{ minWidth: '10rem' }} filter filterPlaceholder="Search by phoneNumber" ></Column>
    ...
    <Column showAddButton={false}  field="createAt" header="Created At" sortable  headerStyle={{ minWidth: '10rem' }} filterField="createAt" dataType="date" filter filterElement={createAtFilterTemplate}  ></Column>
                ...
                <Dialog visible={chefDialog} style={{ width: '450px' }} header="Chef Details" modal className="p-fluid" footer={chefDialogFooter} onHide={hideDialog}>
                    
                
    <div className="field">
        <label htmlFor="name">name</label>
         <InputText id="name" value={chef.name} onChange={(e) => onInputChange(e, 'name')}    required className={classNames({ 'p-invalid': submitted && !chef.name })} />
    </div>
            

    <div className="field">
        <label htmlFor="email">email</label>
         <InputText id="email" value={chef.email} onChange={(e) => onInputChange(e, 'email')}    required className={classNames({ 'p-invalid': submitted && !chef.email })} />
    </div>
            

    <div className="field">
        <label htmlFor="expertise">Expertise</label>
         <MultiSelect   id="expertise"  optionLabel="name" optionValue="name"  value={chef.expertise} options={datamealItems} onChange={(e) => onInputChange(e, 'expertise')}  />
    </div>
            

    <div className="field">
        <label htmlFor="address">address</label>
         <InputText id="address" value={chef.address} onChange={(e) => onInputChange(e, 'address')}    />
    </div>
            

    <div className="field">
        <label htmlFor="phoneNumber">phoneNumber</label>
         <InputText id="phoneNumber" value={chef.phoneNumber} onChange={(e) => onInputChange(e, 'phoneNumber')}    />
    </div>
            

                </Dialog>

                <Dialog visible={deleteChefDialog} style={{ width: '450px' }} header="Confirm" modal footer={deleteChefDialogFooter} onHide={hideDeleteChefDialog}>
                ...
                <Dialog visible={deleteChefsDialog} style={{ width: '450px' }} header="Confirm" modal footer={deleteChefsDialogFooter} onHide={hideDeleteChefsDialog}>
                    <div className="flex align-items-center justify-content-center">
                        <i className="pi pi-exclamation-triangle mr-3" style={{ fontSize: '2rem' }} />
                        {chef && <span>Are you sure you want to delete the selected Chef?</span>}
                    </div>
                </Dialog>
    ...
    <Dialog visible={uploadDialog} style={{ width: '450px' }} header={`Upload ${uploadInfo?.dbColName}`} modal  onHide={hideUploadDialog}>
        <div className="flex align-items-center justify-content-center">
        <CustomFileUpload onUpload={(e)=>updateFileName(e,uploadInfo?.dbColName as keyof Chef)} url={uploadInfo?.url} table="chef" tableId={chef.id } maxFileSize={1000000} accept={uploadInfo?.accept} fieldName="uploadFile" dbColName={uploadInfo?.dbColName} />
        </div>
    </Dialog>  
