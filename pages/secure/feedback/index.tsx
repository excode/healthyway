...
import {Feedback,FeedbackQuery,FeedbackKey, FeedbackService } from '@services/Feedback';
...
customerId: { operator: FilterOperator.AND, constraints: [{ value: null, matchMode: FilterMatchMode.EQUALS }] },
...
return <Calendar value={options.value} onChange={(e) => options.filterCallback(e.value, options.index)} showTime hourFormat="12" />;
    ...
    return (
            <>
                ...
                <Dropdown value={options.value} options={datacustomers}  onChange={(e) => options.filterCallback(e.value)} optionLabel="name" optionValue="email" placeholder="Any" className="p-column-filter" />
            </>
        );
    }
        ...
        if (feedback.id) {
            ...
            let d=  await feedbackService.updateFeedback(_feedback);
                if(d.error==undefined){
                    ...
                    toast.current?.show({ severity: 'success', summary: 'Loaded', detail: 'Feedback Updated', life: 3000 });
                }else{
...
const deleteFeedback = async() => {
    ...
    if(d.error==undefined){
        ...
        setDeleteFeedbackDialog(false);
        setFeedback(emptyFeedback);
        toast.current?.show({ severity: 'warn', summary: 'Deleted', detail: 'Feedback Deleted', life: 3000 });
    }else{
...
const deleteSelectedFeedbacks = () => {
    let _feedbacks = feedbacks.filter((val) => !selectedFeedbacks.includes(val));
    setFeedbacks(_feedbacks);
    setDeleteFeedbacksDialog(false);
    setSelectedFeedbacks([]);
    toast.current?.show({ severity: 'success', summary: 'Successful', detail: 'Feedbacks Deleted', life: 3000 });
};
...
const localFilter=(val:string)=>{
    ...
    if(val.length>1){
        ...
        let filtered = _feedbacks.filter((data) =>  JSON.stringify(data).toLowerCase().indexOf(val.toLowerCase()) !== -1);
    ...
    return (
        <React.Fragment>
            <div className="my-2">
                ...
                <Button label="Delete" icon="pi pi-trash" className="p-button-danger" onClick={confirmDeleteSelected} disabled={!selectedFeedbacks || !selectedFeedbacks.length} />
            </div>
        </React.Fragment>
    );
};
...
const actionBodyTemplate = (rowData:Feedback) => {
    return (
        <>
            <Button icon="pi pi-pencil" className="p-button-rounded p-button-success mr-2" onClick={() => editFeedback(rowData)} />
            <Button icon="pi pi-trash" className="p-button-rounded p-button-warning" onClick={() => confirmDeleteFeedback(rowData)} />
...
const feedbackDialogFooter = (
    <>
        ...
        <Button label="Save" icon="pi pi-check" className="p-button-text" onClick={saveFeedback} />
    </>
);
const deleteFeedbackDialogFooter = (
    <>
        <Button label="No" icon="pi pi-times" className="p-button-text" onClick={hideDeleteFeedbackDialog} />
        <Button label="Yes" icon="pi pi-check" className="p-button-text" onClick={deleteFeedback} />
    </>
);
const deleteFeedbacksDialogFooter = (
    <>
        <Button label="No" icon="pi pi-times" className="p-button-text" onClick={hideDeleteFeedbacksDialog} />
        <Button label="Yes" icon="pi pi-check" className="p-button-text" onClick={deleteSelectedFeedbacks} />
    </>
);
...
return (
        ...
        <div className="col-12">
            <div className="card">
                ...
                <DataTable
                    ...
                    onSelectionChange={(e) => setSelectedFeedbacks(e.value as Feedback[])}
                    dataKey="id"
                    loading={loading}
                    filters={filters1}
                    showGridlines
    ...
    <Column showAddButton={false}  field="customerId" header="customerId" sortable  headerStyle={{ minWidth: '10rem' }} filterField="customerId"   filter filterElement={customerIdFilterTemplate}  ></Column>
    ...
    <Column showAddButton={false}  field="rating" header="rating" sortable  headerStyle={{ minWidth: '10rem' }} dataType="numeric"  filter filterElement={ratingFilterTemplate} ></Column>
    ...
    <Column showAddButton={false}  field="kitchen" header="Kitchen" sortable  headerStyle={{ minWidth: '10rem' }} filter filterPlaceholder="Search by kitchen" ></Column>
                ...
                <Dialog visible={feedbackDialog} style={{ width: '450px' }} header="Feedback Details" modal className="p-fluid" footer={feedbackDialogFooter} onHide={hideDialog}>
                    
                
    <div className="field">
        <label htmlFor="customerId">customerId</label>
         <Dropdown   id="customerId"  optionLabel="name" optionValue="email"  value={feedback.customerId} options={datacustomers} onChange={(e) => onInputChange(e, 'customerId')}  />
    </div>
            

    <div className="field">
        <label htmlFor="chefId">chefId</label>
         <AutoComplete field="name" id="chefId"  completeMethod={searchChef} value={feedback.chefId} suggestions={sugchefs} onChange={(e) => onInputChange(e, 'chefId')}  />
    </div>
            

    <div className="field">
        <label htmlFor="orderId">orderId</label>
         <AutoComplete field="invoiceNo" id="orderId"  completeMethod={searchMealOrder} value={feedback.orderId} suggestions={sugmealOrders} onChange={(e) => onInputChange(e, 'orderId')}  />
    </div>
            

    <div className="field">
        <label htmlFor="rating">rating</label>
         <InputNumber id="rating" value={feedback.rating} onValueChange={(e) => onInputNumberChange(e, 'rating')}  />
    </div>
            

    <div className="field">
        <label htmlFor="comment">comment</label>
         <InputText id="comment" value={feedback.comment} onChange={(e) => onInputChange(e, 'comment')}    required className={classNames({ 'p-invalid': submitted && !feedback.comment })} />
    </div>
            
                </Dialog>

                <Dialog visible={deleteFeedbackDialog} style={{ width: '450px' }} header="Confirm" modal footer={deleteFeedbackDialogFooter} onHide={hideDeleteFeedbackDialog}>
                ...
                <Dialog visible={deleteFeedbacksDialog} style={{ width: '450px' }} header="Confirm" modal footer={deleteFeedbacksDialogFooter} onHide={hideDeleteFeedbacksDialog}>
                    <div className="flex align-items-center justify-content-center">
                        <i className="pi pi-exclamation-triangle mr-3" style={{ fontSize: '2rem' }} />
                        {feedback && <span>Are you sure you want to delete the selected Feedback?</span>}
                    </div>
                </Dialog>
