...
import {Promotions,PromotionsQuery,PromotionsKey, PromotionsService } from '@services/Promotions';
...
{id:'startDate',type:validate.datetime,max:0,min:0,required:true},
...
const initFilters1 = () => {
    setFilters1({
        ...
        mealPlan: { operator: FilterOperator.AND, constraints: [{ value: null, matchMode: FilterMatchMode.IN }] },
...
discount: { operator: FilterOperator.AND, constraints: [{ value: null, matchMode: FilterMatchMode.CONTAINS }] },
kitchen: { operator: FilterOperator.AND, constraints: [{ value: null, matchMode: FilterMatchMode.CONTAINS }] }
    ...
    const searchMealPlan=async(e:any)=>{
        if(e.query.trim().length>1){
            let dataMealPlan_=  await mealplanService.getMealPlanSuggestions(e.query.trim());
...
const mealPlanFilterTemplate = (options:any) => {
    return <AutoComplete field="name" value={options.value} multiple completeMethod={searchMealPlan} suggestions={sugmealPlans} onChange={(e) => options.filterCallback(e.value, options.index)}  placeholder="Select a mealPlan" className="p-column-filter"  />;
            };
        ...
        if (promotions.id) {
            ...
            let d=  await promotionsService.updatePromotions(_promotions);
                if(d.error==undefined){
                    ...
                    toast.current?.show({ severity: 'success', summary: 'Loaded', detail: 'Promotions Updated', life: 3000 });
                }else{
...
const deletePromotions = async() => {
    ...
    if(d.error==undefined){
        ...
        setDeletePromotionsDialog(false);
        setPromotions(emptyPromotions);
        toast.current?.show({ severity: 'warn', summary: 'Deleted', detail: 'Promotions Deleted', life: 3000 });
    }else{
...
const deleteSelectedPromotionss = () => {
    let _promotionss = promotionss.filter((val) => !selectedPromotionss.includes(val));
    setPromotionss(_promotionss);
    setDeletePromotionssDialog(false);
    setSelectedPromotionss([]);
    toast.current?.show({ severity: 'success', summary: 'Successful', detail: 'Promotionss Deleted', life: 3000 });
};
...
const localFilter=(val:string)=>{
    ...
    if(val.length>1){
        let _promotionss =[...promotionss];
        let filtered = _promotionss.filter((data) =>  JSON.stringify(data).toLowerCase().indexOf(val.toLowerCase()) !== -1);
    ...
    return (
        <React.Fragment>
            <div className="my-2">
                ...
                <Button label="Delete" icon="pi pi-trash" className="p-button-danger" onClick={confirmDeleteSelected} disabled={!selectedPromotionss || !selectedPromotionss.length} />
            </div>
        </React.Fragment>
    );
};
...
const actionBodyTemplate = (rowData:Promotions) => {
    return (
        <>
            <Button icon="pi pi-pencil" className="p-button-rounded p-button-success mr-2" onClick={() => editPromotions(rowData)} />
            <Button icon="pi pi-trash" className="p-button-rounded p-button-warning" onClick={() => confirmDeletePromotions(rowData)} />
...
const promotionsDialogFooter = (
    <>
        ...
        <Button label="Save" icon="pi pi-check" className="p-button-text" onClick={savePromotions} />
    </>
);
const deletePromotionsDialogFooter = (
    <>
        <Button label="No" icon="pi pi-times" className="p-button-text" onClick={hideDeletePromotionsDialog} />
        <Button label="Yes" icon="pi pi-check" className="p-button-text" onClick={deletePromotions} />
    </>
);
const deletePromotionssDialogFooter = (
    <>
        <Button label="No" icon="pi pi-times" className="p-button-text" onClick={hideDeletePromotionssDialog} />
        <Button label="Yes" icon="pi pi-check" className="p-button-text" onClick={deleteSelectedPromotionss} />
    </>
);
...
return (
        ...
        <div className="col-12">
            <div className="card">
                ...
                <DataTable
                    ...
                    onSelectionChange={(e) => setSelectedPromotionss(e.value as Promotions[])}
                    dataKey="id"
                    loading={loading}
                    filters={filters1}
                    showGridlines
    ...
    <Column showAddButton={false}  field="mealPlan" header="Meal Plan" sortable  headerStyle={{ minWidth: '10rem' }} filterField="mealPlan"   filter filterElement={mealPlanFilterTemplate}  ></Column>
    ...
    <Column showAddButton={false}  field="discount" header="Discount" sortable  headerStyle={{ minWidth: '10rem' }} filter filterPlaceholder="Search by discount" ></Column>
            
                    <Column body={actionBodyTemplate} headerStyle={{ minWidth: '10rem' }}></Column>
                </DataTable>

                <Dialog visible={promotionsDialog} style={{ width: '450px' }} header="Promotions Details" modal className="p-fluid" footer={promotionsDialogFooter} onHide={hideDialog}>
                    
                

    <div className="field">
        <label htmlFor="name">Name</label>
         <InputText id="name" value={promotions.name} onChange={(e) => onInputChange(e, 'name')}    required className={classNames({ 'p-invalid': submitted && !promotions.name })} />
    </div>
            

    <div className="field">
        <label htmlFor="description">Description</label>
         <InputText id="description" value={promotions.description} onChange={(e) => onInputChange(e, 'description')}    required className={classNames({ 'p-invalid': submitted && !promotions.description })} />
    </div>
            

    <div className="field">
        <label htmlFor="startDate">Start Date</label>
         <Calendar id="startDate" value={promotions.startDate?new Date(promotions.startDate):null }  onChange={(e) => onInputChange(e, 'startDate')} showTime hourFormat="12"     required className={classNames({ 'p-invalid': submitted && !promotions.startDate })} />
    </div>
            

    <div className="field">
        <label htmlFor="endDate">End Date</label>
         <Calendar id="endDate" value={promotions.endDate?new Date(promotions.endDate):null }  onChange={(e) => onInputChange(e, 'endDate')} showTime hourFormat="12"     required className={classNames({ 'p-invalid': submitted && !promotions.endDate })} />
    </div>
            

    <div className="field">
        <label htmlFor="discount">Discount</label>
         <InputText id="discount" value={promotions.discount} onChange={(e) => onInputChange(e, 'discount')}    required className={classNames({ 'p-invalid': submitted && !promotions.discount })} />
    </div>
            

                </Dialog>

                <Dialog visible={deletePromotionsDialog} style={{ width: '450px' }} header="Confirm" modal footer={deletePromotionsDialogFooter} onHide={hideDeletePromotionsDialog}>
                ...
                <Dialog visible={deletePromotionssDialog} style={{ width: '450px' }} header="Confirm" modal footer={deletePromotionssDialogFooter} onHide={hideDeletePromotionssDialog}>
                    <div className="flex align-items-center justify-content-center">
                        <i className="pi pi-exclamation-triangle mr-3" style={{ fontSize: '2rem' }} />
                        {promotions && <span>Are you sure you want to delete the selected Promotions?</span>}
                    </div>
                </Dialog>
