...
import {Subscription,SubscriptionQuery,SubscriptionKey, SubscriptionService } from '@services/Subscription';
...
const [backupSubscriptions, setBackupSubscriptions] =  useState<Subscription[]>([]);
...
const initFilters1 = () => {
    setFilters1({
        ...
        customerId: { operator: FilterOperator.AND, constraints: [{ value: null, matchMode: FilterMatchMode.IN }] },
...
breakfast: { operator: FilterOperator.AND, constraints: [{ value: null, matchMode: FilterMatchMode.IN }] },
lunch: { operator: FilterOperator.AND, constraints: [{ value: null, matchMode: FilterMatchMode.IN }] },
dinner: { operator: FilterOperator.AND, constraints: [{ value: null, matchMode: FilterMatchMode.IN }] },
...
const customerIdFilterTemplate = (options:any) => {
    return <AutoComplete field="name" value={options.value}  completeMethod={searchCustomer} suggestions={sugcustomers} onChange={(e) => options.filterCallback(e.value, options.index)}  placeholder="Select a customerId" className="p-column-filter"  />;
            };
...
const breakfastFilterTemplate = (options:any) => {
    return <AutoComplete field="name" value={options.value} multiple completeMethod={searchMealItem} suggestions={sugmealItems} onChange={(e) => options.filterCallback(e.value, options.index)}  placeholder="Select a breakfast" className="p-column-filter"  />;
            };
            
const lunchFilterTemplate = (options:any) => {
    return <AutoComplete field="name" value={options.value} multiple completeMethod={searchMealItem} suggestions={sugmealItems} onChange={(e) => options.filterCallback(e.value, options.index)}  placeholder="Select a lunch" className="p-column-filter"  />;
            };
            
const dinnerFilterTemplate = (options:any) => {
    return <AutoComplete field="name" value={options.value} multiple completeMethod={searchMealItem} suggestions={sugmealItems} onChange={(e) => options.filterCallback(e.value, options.index)}  placeholder="Select a dinner" className="p-column-filter"  />;
            };
        ...
        if (subscription.id) {
            ...
            let d=  await subscriptionService.updateSubscription(_subscription);
                if(d.error==undefined){
                    ...
                    const index = _subscriptions.findIndex(c => c.id === subscription.id)
                    if (index !== -1) {
                        _subscriptions[index] = {..._subscription,startDate:_subscription.startDate.toString(),endDate:_subscription.endDate.toString()};
                       // _subscriptions[index] = _subscription;
                        //_subscriptions.splice(index, 1, {..._subscription,id:id});
                    }
                    toast.current?.show({ severity: 'success', summary: 'Loaded', detail: 'Subscription Updated', life: 3000 });
                }else{
...
const deleteSubscription = async() => {
    ...
    if(d.error==undefined){
        let _subscriptions = subscriptions.filter((val) => val.id !== subscription.id);
        setSubscriptions(_subscriptions);
        setBackupSubscriptions(_subscriptions);
        setDeleteSubscriptionDialog(false);
        setSubscription(emptySubscription);
        toast.current?.show({ severity: 'warn', summary: 'Deleted', detail: 'Subscription Deleted', life: 3000 });
    }else{
...
const localFilter=(val:string)=>{
    ...
    if(val.length>1){
        ...
        let filtered = _subscriptions.filter((data) =>  JSON.stringify(data).toLowerCase().indexOf(val.toLowerCase()) !== -1);
...
const actionBodyTemplate = (rowData:Subscription) => {
    return (
        <>
            <Button icon="pi pi-pencil" className="p-button-rounded p-button-success mr-2" onClick={() => editSubscription(rowData)} />
            <Button icon="pi pi-trash" className="p-button-rounded p-button-warning" onClick={() => confirmDeleteSubscription(rowData)} />
...
return (
        ...
        <div className="col-12">
            <div className="card">
                ...
                <DataTable
                    ...
                    onSelectionChange={(e) => setSelectedSubscriptions(e.value as Subscription[])}
                    dataKey="id"
                    loading={loading}
                    filters={filters1}
                    showGridlines
    ...
    <Column showAddButton={false}  field="customerId" header="Customer Id" sortable  headerStyle={{ minWidth: '10rem' }} filterField="customerId"   filter filterElement={customerIdFilterTemplate}  ></Column>
            


    <Column showAddButton={false}  field="startDate" header="Start Date" sortable  headerStyle={{ minWidth: '10rem' }} filterField="startDate" dataType="date" filter filterElement={startDateFilterTemplate}  ></Column>
            

    <Column showAddButton={false}  field="endDate" header="End Date" sortable  headerStyle={{ minWidth: '10rem' }} filterField="endDate" dataType="date" filter filterElement={endDateFilterTemplate}  ></Column>
    ...
    <div className="field">
        <label htmlFor="customerId">Customer Id</label>
         <AutoComplete field="name" id="customerId"  completeMethod={searchCustomer} value={subscription.customerId} suggestions={sugcustomers} onChange={(e) => onInputChange(e, 'customerId')}  />
    </div>
            

    <div className="field">
        <label htmlFor="startDate">Start Date</label>
         <Calendar id="startDate" value={subscription.startDate?new Date(subscription.startDate):null }  onChange={(e) => onInputChange(e, 'startDate')} dateFormat="mm/dd/yy" placeholder="mm/dd/yyyy" mask="99/99/9999"     required className={classNames({ 'p-invalid': submitted && !subscription.startDate })} />
    </div>
            

    <div className="field">
        <label htmlFor="endDate">End Date</label>
         <Calendar id="endDate" value={subscription.endDate?new Date(subscription.endDate):null }  onChange={(e) => onInputChange(e, 'endDate')} dateFormat="mm/dd/yy" placeholder="mm/dd/yyyy" mask="99/99/9999"     required className={classNames({ 'p-invalid': submitted && !subscription.endDate })} />
    </div>
            

    <div className="field">
        <label htmlFor="status">Status</label>
         <ListBox   id="status" optionLabel="name"  value={subscription.status} options={datastatuss} onChange={(e) => onInputChange(e, 'status')}  />
    </div>
            

    <div className="field">
        <label htmlFor="breakfast">Breakfast</label>
         <AutoComplete field="name" id="breakfast" multiple completeMethod={searchMealItem} value={subscription.breakfast} suggestions={sugmealItems} onChange={(e) => onInputChange(e, 'breakfast')}  />
    </div>
            

    <div className="field">
        <label htmlFor="lunch">Lunch</label>
         <AutoComplete field="name" id="lunch" multiple completeMethod={searchMealItem} value={subscription.lunch} suggestions={sugmealItems} onChange={(e) => onInputChange(e, 'lunch')}  />
    </div>
            

    <div className="field">
        <label htmlFor="dinner">Dinner</label>
         <AutoComplete field="name" id="dinner" multiple completeMethod={searchMealItem} value={subscription.dinner} suggestions={sugmealItems} onChange={(e) => onInputChange(e, 'dinner')}  />
    </div>
                ...
                <Dialog visible={deleteSubscriptionsDialog} style={{ width: '450px' }} header="Confirm" modal footer={deleteSubscriptionsDialogFooter} onHide={hideDeleteSubscriptionsDialog}>
                    <div className="flex align-items-center justify-content-center">
                        <i className="pi pi-exclamation-triangle mr-3" style={{ fontSize: '2rem' }} />
                        {subscription && <span>Are you sure you want to delete the selected Subscription?</span>}
                    </div>
                </Dialog>
