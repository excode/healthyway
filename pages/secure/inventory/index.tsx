...
import {Inventory,InventoryQuery,InventoryKey, InventoryService } from '@services/Inventory';
...
const initFilters1 = () => {
    setFilters1({
        ...
        itemCode: { operator: FilterOperator.AND, constraints: [{ value: null, matchMode: FilterMatchMode.IN }] },
itemName: { operator: FilterOperator.AND, constraints: [{ value: null, matchMode: FilterMatchMode.CONTAINS }] },
...
kitchen: { operator: FilterOperator.AND, constraints: [{ value: null, matchMode: FilterMatchMode.CONTAINS }] }
    ...
    const searchIngredient=async(e:any)=>{
        if(e.query.trim().length>1){
            let dataIngredient_=  await ingredientService.getIngredientSuggestions(e.query.trim());
...
const itemCodeFilterTemplate = (options:any) => {
    return <AutoComplete field="name" value={options.value} multiple completeMethod={searchIngredient} suggestions={sugingredients} onChange={(e) => options.filterCallback(e.value, options.index)}  placeholder="Select a itemCode" className="p-column-filter"  />;
            };
        ...
        if (inventory.id) {
            ...
            let d=  await inventoryService.updateInventory(_inventory);
                if(d.error==undefined){
                    ...
                    toast.current?.show({ severity: 'success', summary: 'Loaded', detail: 'Inventory Updated', life: 3000 });
                }else{
...
const deleteInventory = async() => {
    ...
    if(d.error==undefined){
        ...
        setDeleteInventoryDialog(false);
        setInventory(emptyInventory);
        toast.current?.show({ severity: 'warn', summary: 'Deleted', detail: 'Inventory Deleted', life: 3000 });
    }else{
...
const deleteSelectedInventorys = () => {
    let _inventorys = inventorys.filter((val) => !selectedInventorys.includes(val));
    setInventorys(_inventorys);
    setDeleteInventorysDialog(false);
    setSelectedInventorys([]);
    toast.current?.show({ severity: 'success', summary: 'Successful', detail: 'Inventorys Deleted', life: 3000 });
};
...
const localFilter=(val:string)=>{
    ...
    if(val.length>1){
        ...
        let filtered = _inventorys.filter((data) =>  JSON.stringify(data).toLowerCase().indexOf(val.toLowerCase()) !== -1);
    ...
    return (
        <React.Fragment>
            <div className="my-2">
                ...
                <Button label="Delete" icon="pi pi-trash" className="p-button-danger" onClick={confirmDeleteSelected} disabled={!selectedInventorys || !selectedInventorys.length} />
            </div>
        </React.Fragment>
    );
};
...
const actionBodyTemplate = (rowData:Inventory) => {
    return (
        <>
            <Button icon="pi pi-pencil" className="p-button-rounded p-button-success mr-2" onClick={() => editInventory(rowData)} />
            <Button icon="pi pi-trash" className="p-button-rounded p-button-warning" onClick={() => confirmDeleteInventory(rowData)} />
...
const inventoryDialogFooter = (
    <>
        ...
        <Button label="Save" icon="pi pi-check" className="p-button-text" onClick={saveInventory} />
    </>
);
const deleteInventoryDialogFooter = (
    <>
        <Button label="No" icon="pi pi-times" className="p-button-text" onClick={hideDeleteInventoryDialog} />
        <Button label="Yes" icon="pi pi-check" className="p-button-text" onClick={deleteInventory} />
    </>
);
const deleteInventorysDialogFooter = (
    <>
        <Button label="No" icon="pi pi-times" className="p-button-text" onClick={hideDeleteInventorysDialog} />
        <Button label="Yes" icon="pi pi-check" className="p-button-text" onClick={deleteSelectedInventorys} />
    </>
);
...
return (
        ...
        <div className="col-12">
            <div className="card">
                ...
                <DataTable
                    ...
                    onSelectionChange={(e) => setSelectedInventorys(e.value as Inventory[])}
                    dataKey="id"
                    loading={loading}
                    filters={filters1}
                    showGridlines
    ...
    <Column showAddButton={false}  field="itemCode" header="Item Code" sortable  headerStyle={{ minWidth: '10rem' }} filterField="itemCode"   filter filterElement={itemCodeFilterTemplate}  ></Column>
            

    <Column showAddButton={false}  field="itemName" header="Item Name" sortable  headerStyle={{ minWidth: '10rem' }} filter filterPlaceholder="Search by itemName" ></Column>
    ...
    <Column showAddButton={false}  field="note" header="Notes" sortable  headerStyle={{ minWidth: '10rem' }} filter filterPlaceholder="Search by note" ></Column>
    ...
    <Column showAddButton={false}  field="createBy" header="Created By" sortable  headerStyle={{ minWidth: '10rem' }} filter filterPlaceholder="Search by createBy" ></Column>
                ...
                <Dialog visible={inventoryDialog} style={{ width: '450px' }} header="Inventory Details" modal className="p-fluid" footer={inventoryDialogFooter} onHide={hideDialog}>
                    
                
    <div className="field">
        <label htmlFor="itemCode">Item Code</label>
         <AutoComplete field="name" id="itemCode" multiple completeMethod={searchIngredient} value={inventory.itemCode} suggestions={sugingredients} onChange={(e) => onInputChange(e, 'itemCode')}  />
    </div>
            

    <div className="field">
        <label htmlFor="itemName">Item Name</label>
         <InputText id="itemName" value={inventory.itemName} onChange={(e) => onInputChange(e, 'itemName')}    />
    </div>
            

    <div className="field">
        <label htmlFor="quantity">quantity</label>
         <InputNumber id="quantity" value={inventory.quantity} onValueChange={(e) => onInputNumberChange(e, 'quantity')}  />
    </div>
            

    <div className="field">
        <label htmlFor="note">Notes</label>
         <InputText id="note" value={inventory.note} onChange={(e) => onInputChange(e, 'note')}    />
    </div>
            
                </Dialog>

                <Dialog visible={deleteInventoryDialog} style={{ width: '450px' }} header="Confirm" modal footer={deleteInventoryDialogFooter} onHide={hideDeleteInventoryDialog}>
                ...
                <Dialog visible={deleteInventorysDialog} style={{ width: '450px' }} header="Confirm" modal footer={deleteInventorysDialogFooter} onHide={hideDeleteInventorysDialog}>
                    <div className="flex align-items-center justify-content-center">
                        <i className="pi pi-exclamation-triangle mr-3" style={{ fontSize: '2rem' }} />
                        {inventory && <span>Are you sure you want to delete the selected Inventory?</span>}
                    </div>
                </Dialog>
