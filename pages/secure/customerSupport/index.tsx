...
import {CustomerSupport,CustomerSupportQuery,CustomerSupportKey, CustomerSupportService } from '@services/CustomerSupport';
...
{id:'description',type:validate.text,max:500,min:0,required:true},
...
const [backupCustomerSupports, setBackupCustomerSupports] =  useState<CustomerSupport[]>([]);
const [loading,setLoading] = useState(false);
const [customerSupportDialog, setCustomerSupportDialog] = useState(false);
const [deleteCustomerSupportDialog, setDeleteCustomerSupportDialog] = useState(false);
const [deleteCustomerSupportsDialog, setDeleteCustomerSupportsDialog] = useState(false);
const [customerSupport, setCustomerSupport] = useState<CustomerSupport>(emptyCustomerSupport);
const [selectedCustomerSupports, setSelectedCustomerSupports] = useState<CustomerSupport[]>([]);
...
const saveCustomerSupport = async () => {
    setSubmitted(true);
    const validationErrors:string[]=validateForm(customerSupport,validation)
        ...
        if (customerSupport.id) {
            ...
            let d=  await customersupportService.updateCustomerSupport(_customerSupport);
                if(d.error==undefined){
                    
                    const index = _customerSupports.findIndex(c => c.id === customerSupport.id)
                    if (index !== -1) {
                        _customerSupports[index] = {..._customerSupport};
                       // _customerSupports[index] = _customerSupport;
                        //_customerSupports.splice(index, 1, {..._customerSupport,id:id});
                    }
                    toast.current?.show({ severity: 'success', summary: 'Loaded', detail: 'CustomerSupport Updated', life: 3000 });
                }else{
            ...
            let d=  await customersupportService.addCustomerSupport(_customerSupport);
            if(d.error==undefined){
                var newID= d.id
...
const deleteCustomerSupport = async() => {
    ...
    let d=  await customersupportService.deleteCustomerSupport(customerSupport.id??'');
    if(d.error==undefined){
        let _customerSupports = customerSupports.filter((val) => val.id !== customerSupport.id);
        setCustomerSupports(_customerSupports);
        setBackupCustomerSupports(_customerSupports);
        setDeleteCustomerSupportDialog(false);
        setCustomerSupport(emptyCustomerSupport);
        toast.current?.show({ severity: 'warn', summary: 'Deleted', detail: 'CustomerSupport Deleted', life: 3000 });
    }else{
...
const deleteSelectedCustomerSupports = () => {
    let _customerSupports = customerSupports.filter((val) => !selectedCustomerSupports.includes(val));
    setCustomerSupports(_customerSupports);
    setDeleteCustomerSupportsDialog(false);
    setSelectedCustomerSupports([]);
    toast.current?.show({ severity: 'success', summary: 'Successful', detail: 'CustomerSupports Deleted', life: 3000 });
};
...
const localFilter=(val:string)=>{
    ...
    if(val.length>1){
        ...
        let filtered = _customerSupports.filter((data) =>  JSON.stringify(data).toLowerCase().indexOf(val.toLowerCase()) !== -1);
    ...
    return (
        <React.Fragment>
            <div className="my-2">
                ...
                <Button label="Delete" icon="pi pi-trash" className="p-button-danger" onClick={confirmDeleteSelected} disabled={!selectedCustomerSupports || !selectedCustomerSupports.length} />
            </div>
        </React.Fragment>
    );
};
...
const actionBodyTemplate = (rowData:CustomerSupport) => {
    return (
        <>
            <Button icon="pi pi-pencil" className="p-button-rounded p-button-success mr-2" onClick={() => editCustomerSupport(rowData)} />
            <Button icon="pi pi-trash" className="p-button-rounded p-button-warning" onClick={() => confirmDeleteCustomerSupport(rowData)} />
...
const customerSupportDialogFooter = (
    <>
        ...
        <Button label="Save" icon="pi pi-check" className="p-button-text" onClick={saveCustomerSupport} />
    </>
);
const deleteCustomerSupportDialogFooter = (
    <>
        <Button label="No" icon="pi pi-times" className="p-button-text" onClick={hideDeleteCustomerSupportDialog} />
        <Button label="Yes" icon="pi pi-check" className="p-button-text" onClick={deleteCustomerSupport} />
    </>
);
const deleteCustomerSupportsDialogFooter = (
    <>
        <Button label="No" icon="pi pi-times" className="p-button-text" onClick={hideDeleteCustomerSupportsDialog} />
        <Button label="Yes" icon="pi pi-check" className="p-button-text" onClick={deleteSelectedCustomerSupports} />
    </>
);
...
return (
        ...
        <div className="col-12">
            <div className="card">
                ...
                <DataTable
                    ...
                    onSelectionChange={(e) => setSelectedCustomerSupports(e.value as CustomerSupport[])}
                    dataKey="id"
                    loading={loading}
                    filters={filters1}
                    showGridlines
    ...
    <Column showAddButton={false}  field="customerName" header="Name" sortable  headerStyle={{ minWidth: '10rem' }} filterField="customerName"   filter filterElement={customerNameFilterTemplate}  ></Column>
    ...
    <Column showAddButton={false}  field="customerPhone" header="Phone" sortable  headerStyle={{ minWidth: '10rem' }} filterField="customerPhone"   filter filterElement={customerPhoneFilterTemplate}  ></Column>
    ...
    <Column showAddButton={false}  field="createAt" header="Created At" sortable  headerStyle={{ minWidth: '10rem' }} filterField="createAt" dataType="date" filter filterElement={createAtFilterTemplate}  ></Column>
                ...
                <Dialog visible={customerSupportDialog} style={{ width: '450px' }} header="CustomerSupport Details" modal className="p-fluid" footer={customerSupportDialogFooter} onHide={hideDialog}>
                    
                
    <div className="field">
        <label htmlFor="customerName">Name</label>
         <AutoComplete field="name" id="customerName"  completeMethod={searchCustomer} value={customerSupport.customerName} suggestions={sugcustomers} onChange={(e) => onInputChange(e, 'customerName')}  />
    </div>
            

    <div className="field">
        <label htmlFor="email">Email</label>
         <InputText id="email" value={customerSupport.email} onChange={(e) => onInputChange(e, 'email')}    />
    </div>
            

    <div className="field">
        <label htmlFor="subject">subject</label>
         <InputText id="subject" value={customerSupport.subject} onChange={(e) => onInputChange(e, 'subject')}    required className={classNames({ 'p-invalid': submitted && !customerSupport.subject })} />
    </div>
            

    <div className="field">
        <label htmlFor="description">Description</label>
         <InputText id="description" value={customerSupport.description} onChange={(e) => onInputChange(e, 'description')}    required className={classNames({ 'p-invalid': submitted && !customerSupport.description })} />
    </div>
            

    <div className="field">
        <label htmlFor="status">status</label>
         <ListBox   id="status" optionLabel="name"  value={customerSupport.status} options={datastatuss} onChange={(e) => onInputChange(e, 'status')}  />
    </div>
            

    <div className="field">
        <label htmlFor="customerPhone">Phone</label>
         <AutoComplete field="mobile" id="customerPhone"  completeMethod={searchCustomer} value={customerSupport.customerPhone} suggestions={sugcustomers} onChange={(e) => onInputChange(e, 'customerPhone')}  />
    </div>
            
                </Dialog>

                <Dialog visible={deleteCustomerSupportDialog} style={{ width: '450px' }} header="Confirm" modal footer={deleteCustomerSupportDialogFooter} onHide={hideDeleteCustomerSupportDialog}>
                ...
                <Dialog visible={deleteCustomerSupportsDialog} style={{ width: '450px' }} header="Confirm" modal footer={deleteCustomerSupportsDialogFooter} onHide={hideDeleteCustomerSupportsDialog}>
                    <div className="flex align-items-center justify-content-center">
                        <i className="pi pi-exclamation-triangle mr-3" style={{ fontSize: '2rem' }} />
                        {customerSupport && <span>Are you sure you want to delete the selected CustomerSupport?</span>}
                    </div>
                </Dialog>
