...
import {Notification,NotificationQuery,NotificationKey, NotificationService } from '@services/Notification';
...
const [backupNotifications, setBackupNotifications] =  useState<Notification[]>([]);
...
message: { operator: FilterOperator.AND, constraints: [{ value: null, matchMode: FilterMatchMode.CONTAINS }] },
        ...
        if (notification.id) {
            ...
            let d=  await notificationService.updateNotification(_notification);
                if(d.error==undefined){
                    ...
                    const index = _notifications.findIndex(c => c.id === notification.id)
                    if (index !== -1) {
                        _notifications[index] = {..._notification};
                       // _notifications[index] = _notification;
                        //_notifications.splice(index, 1, {..._notification,id:id});
                    }
                    toast.current?.show({ severity: 'success', summary: 'Loaded', detail: 'Notification Updated', life: 3000 });
                }else{
...
const deleteNotification = async() => {
    ...
    if(d.error==undefined){
        ...
        setDeleteNotificationDialog(false);
        setNotification(emptyNotification);
        toast.current?.show({ severity: 'warn', summary: 'Deleted', detail: 'Notification Deleted', life: 3000 });
    }else{
...
const deleteSelectedNotifications = () => {
    let _notifications = notifications.filter((val) => !selectedNotifications.includes(val));
    setNotifications(_notifications);
    setDeleteNotificationsDialog(false);
    setSelectedNotifications([]);
    toast.current?.show({ severity: 'success', summary: 'Successful', detail: 'Notifications Deleted', life: 3000 });
};
...
const localFilter=(val:string)=>{
    ...
    if(val.length>1){
        ...
        let filtered = _notifications.filter((data) =>  JSON.stringify(data).toLowerCase().indexOf(val.toLowerCase()) !== -1);
    ...
    return (
        <React.Fragment>
            <div className="my-2">
                ...
                <Button label="Delete" icon="pi pi-trash" className="p-button-danger" onClick={confirmDeleteSelected} disabled={!selectedNotifications || !selectedNotifications.length} />
            </div>
        </React.Fragment>
    );
};
...
const actionBodyTemplate = (rowData:Notification) => {
    return (
        <>
            <Button icon="pi pi-pencil" className="p-button-rounded p-button-success mr-2" onClick={() => editNotification(rowData)} />
            <Button icon="pi pi-trash" className="p-button-rounded p-button-warning" onClick={() => confirmDeleteNotification(rowData)} />
...
const deleteNotificationsDialogFooter = (
    <>
        <Button label="No" icon="pi pi-times" className="p-button-text" onClick={hideDeleteNotificationsDialog} />
        <Button label="Yes" icon="pi pi-check" className="p-button-text" onClick={deleteSelectedNotifications} />
    </>
);
...
return (
    ...
    <Column showAddButton={false}  field="message" header="message" sortable  headerStyle={{ minWidth: '10rem' }} filter filterPlaceholder="Search by message" ></Column>
    ...
    <Column showAddButton={false}  field="createAt" header="Created At" sortable  headerStyle={{ minWidth: '10rem' }} filterField="createAt" dataType="date" filter filterElement={createAtFilterTemplate}  ></Column>
                ...
                <Dialog visible={notificationDialog} style={{ width: '450px' }} header="Notification Details" modal className="p-fluid" footer={notificationDialogFooter} onHide={hideDialog}>
                    
                
    <div className="field">
        <label htmlFor="chefId">chefId</label>
         <AutoComplete field="name" id="chefId"  completeMethod={searchChef} value={notification.chefId} suggestions={sugchefs} onChange={(e) => onInputChange(e, 'chefId')}  />
    </div>
            

    <div className="field">
        <label htmlFor="message">message</label>
         <InputText id="message" value={notification.message} onChange={(e) => onInputChange(e, 'message')}    required className={classNames({ 'p-invalid': submitted && !notification.message })} />
    </div>
            

    <div className="field">
        <label htmlFor="status">Status</label>
         <Dropdown   id="status" optionLabel="name"  value={notification.status} options={datastatuss} onChange={(e) => onInputChange(e, 'status')}  />
    </div>
                ...
                <Dialog visible={deleteNotificationsDialog} style={{ width: '450px' }} header="Confirm" modal footer={deleteNotificationsDialogFooter} onHide={hideDeleteNotificationsDialog}>
                    <div className="flex align-items-center justify-content-center">
                        <i className="pi pi-exclamation-triangle mr-3" style={{ fontSize: '2rem' }} />
                        {notification && <span>Are you sure you want to delete the selected Notification?</span>}
                    </div>
                </Dialog>
