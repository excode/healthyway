...
import {Ingredient,IngredientQuery,IngredientKey, IngredientService } from '@services/Ingredient';
...
{id:'description',type:validate.text,max:90,min:0,required:false}
    ]
...
itemCode: { operator: FilterOperator.AND, constraints: [{ value: null, matchMode: FilterMatchMode.CONTAINS }] },
        ...
        if (ingredient.id) {
            ...
            let d=  await ingredientService.updateIngredient(_ingredient);
                if(d.error==undefined){
                    ...
                    toast.current?.show({ severity: 'success', summary: 'Loaded', detail: 'Ingredient Updated', life: 3000 });
                }else{
...
const deleteIngredient = async() => {
    ...
    if(d.error==undefined){
        ...
        setDeleteIngredientDialog(false);
        setIngredient(emptyIngredient);
        toast.current?.show({ severity: 'warn', summary: 'Deleted', detail: 'Ingredient Deleted', life: 3000 });
    }else{
...
const deleteSelectedIngredients = () => {
    ...
    toast.current?.show({ severity: 'success', summary: 'Successful', detail: 'Ingredients Deleted', life: 3000 });
};
...
const localFilter=(val:string)=>{
    ...
    if(val.length>1){
        let _ingredients =[...ingredients];
        let filtered = _ingredients.filter((data) =>  JSON.stringify(data).toLowerCase().indexOf(val.toLowerCase()) !== -1);
    ...
    return (
        <React.Fragment>
            <div className="my-2">
                ...
                <Button label="Delete" icon="pi pi-trash" className="p-button-danger" onClick={confirmDeleteSelected} disabled={!selectedIngredients || !selectedIngredients.length} />
            </div>
        </React.Fragment>
    );
};
...
const actionBodyTemplate = (rowData:Ingredient) => {
    return (
        <>
            <Button icon="pi pi-pencil" className="p-button-rounded p-button-success mr-2" onClick={() => editIngredient(rowData)} />
            <Button icon="pi pi-trash" className="p-button-rounded p-button-warning" onClick={() => confirmDeleteIngredient(rowData)} />
...
const ingredientDialogFooter = (
    <>
        ...
        <Button label="Save" icon="pi pi-check" className="p-button-text" onClick={saveIngredient} />
    </>
);
...
const deleteIngredientsDialogFooter = (
    <>
        <Button label="No" icon="pi pi-times" className="p-button-text" onClick={hideDeleteIngredientsDialog} />
        <Button label="Yes" icon="pi pi-check" className="p-button-text" onClick={deleteSelectedIngredients} />
    </>
);
...
return (
        ...
        <div className="col-12">
            <div className="card">
                ...
                <DataTable
                    ...
                    onSelectionChange={(e) => setSelectedIngredients(e.value as Ingredient[])}
                    dataKey="id"
                    loading={loading}
                    filters={filters1}
                    showGridlines
    ...
    <Column showAddButton={false}  field="itemCode" header="Item Code" sortable  headerStyle={{ minWidth: '10rem' }} filter filterPlaceholder="Search by itemCode" ></Column>
            

    <Column showAddButton={false}  field="unit" header="unit" sortable  headerStyle={{ minWidth: '10rem' }} filter filterPlaceholder="Search by unit" ></Column>
    ...
    <Column showAddButton={false}  field="createAt" header="Created At" sortable  headerStyle={{ minWidth: '10rem' }} filterField="createAt" dataType="date" filter filterElement={createAtFilterTemplate}  ></Column>
                ...
                <Dialog visible={ingredientDialog} style={{ width: '450px' }} header="Ingredient Details" modal className="p-fluid" footer={ingredientDialogFooter} onHide={hideDialog}>
                    
                
    <div className="field">
        <label htmlFor="name">name</label>
         <InputText id="name" value={ingredient.name} onChange={(e) => onInputChange(e, 'name')}    required className={classNames({ 'p-invalid': submitted && !ingredient.name })} />
    </div>
            

    <div className="field">
        <label htmlFor="itemCode">Item Code</label>
         <InputText id="itemCode" value={ingredient.itemCode} onChange={(e) => onInputChange(e, 'itemCode')}    required className={classNames({ 'p-invalid': submitted && !ingredient.itemCode })} />
    </div>
            

    <div className="field">
        <label htmlFor="unit">unit</label>
         <InputText id="unit" value={ingredient.unit} onChange={(e) => onInputChange(e, 'unit')}    required className={classNames({ 'p-invalid': submitted && !ingredient.unit })} />
    </div>
            

    <div className="field">
        <label htmlFor="description">description</label>
         <InputText id="description" value={ingredient.description} onChange={(e) => onInputChange(e, 'description')}    />
    </div>
            
                </Dialog>

                <Dialog visible={deleteIngredientDialog} style={{ width: '450px' }} header="Confirm" modal footer={deleteIngredientDialogFooter} onHide={hideDeleteIngredientDialog}>
                ...
                <Dialog visible={deleteIngredientsDialog} style={{ width: '450px' }} header="Confirm" modal footer={deleteIngredientsDialogFooter} onHide={hideDeleteIngredientsDialog}>
                    <div className="flex align-items-center justify-content-center">
                        <i className="pi pi-exclamation-triangle mr-3" style={{ fontSize: '2rem' }} />
                        {ingredient && <span>Are you sure you want to delete the selected Ingredient?</span>}
                    </div>
                </Dialog>
