        ...
        const onFilter = (e: React.ChangeEvent<HTMLInputElement>) => {
            ...
            if (value.length === 0) {
                ...
                const filtered = mealPlans!.filter((mealPlan:MealPlan) => {
                    return (mealPlan.createBy.toLowerCase().includes(value)||mealPlan.updateBy.toLowerCase().includes(value)||mealPlan.name.toLowerCase().includes(value)||mealPlan.description.toLowerCase().includes(value)||mealPlan.code.toLowerCase().includes(value)) ;
                });
                    ...
                    {(data.image!=undefined  ) 
                      ...
                      :  <img src={"/photo_na.png"} alt={data.createBy} className="my-4 md:my-0 w-9 md:w-10rem shadow-2 mr-5" />
                        ...
                        <div className="flex flex-row md:flex-column justify-content-between w-full md:w-auto align-items-center md:align-items-end mt-5 md:mt-0">
                            ...
                            <Button icon="pi pi-shopping-cart" label="Add to Cart" disabled={data.quantity === 'OUTOFSTOCK'} className="mb-2 p-button-sm"></Button>
                            <span className={`product-badge status-active`}>{data.quantity}</span>
                        </div>
                    </div>
                </div>
            );
                    ...
                    <div className="card m-3 border-1 surface-border">
                      ...
                      :  <img src={`/photo_na.png`} alt={data.createBy} className="my-4 md:my-0 w-9 md:w-10rem shadow-2 mr-5" />
                        ...
                        <div className="flex align-items-center justify-content-between">
                            ...
                            <Button icon="pi pi-shopping-cart" disabled={data.quantity === 'OUTOFSTOCK'} />
                        </div>
                    </div>
                </div>
            );
