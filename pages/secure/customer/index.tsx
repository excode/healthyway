...
import {Customer,CustomerQuery,CustomerKey, CustomerService } from '@services/Customer';
...
GeoTag: { operator: FilterOperator.AND, constraints: [{ value: null, matchMode: FilterMatchMode.CONTAINS }] },
...
customerType: { operator: FilterOperator.AND, constraints: [{ value: null, matchMode: FilterMatchMode.EQUALS }] }
...
return <Calendar value={options.value} onChange={(e) => options.filterCallback(e.value, options.index)} showTime hourFormat="12" />;
    ...
    return (
            <>
                ...
                <Dropdown value={options.value} options={datacustomerTypes}  onChange={(e) => options.filterCallback(e.value)} optionLabel="name" optionValue="value" placeholder="Any" className="p-column-filter" />
            </>
        );
    }
        ...
        if (customer.id) {
            ...
            let d=  await customerService.updateCustomer(_customer);
                if(d.error==undefined){
                    ...
                    toast.current?.show({ severity: 'success', summary: 'Loaded', detail: 'Customer Updated', life: 3000 });
                }else{
...
const deleteCustomer = async() => {
    ...
    if(d.error==undefined){
        ...
        setDeleteCustomerDialog(false);
        setCustomer(emptyCustomer);
        toast.current?.show({ severity: 'warn', summary: 'Deleted', detail: 'Customer Deleted', life: 3000 });
    }else{
...
const localFilter=(val:string)=>{
    ...
    if(val.length>1){
        ...
        let filtered = _customers.filter((data) =>  JSON.stringify(data).toLowerCase().indexOf(val.toLowerCase()) !== -1);
...
const actionBodyTemplate = (rowData:Customer) => {
    return (
        <>
            <Button icon="pi pi-pencil" className="p-button-rounded p-button-success mr-2" onClick={() => editCustomer(rowData)} />
            <Button icon="pi pi-trash" className="p-button-rounded p-button-warning" onClick={() => confirmDeleteCustomer(rowData)} />
...
const deleteCustomersDialogFooter = (
    <>
        <Button label="No" icon="pi pi-times" className="p-button-text" onClick={hideDeleteCustomersDialog} />
        <Button label="Yes" icon="pi pi-check" className="p-button-text" onClick={deleteSelectedCustomers} />
    </>
);
...
return (
    ...
    <Column showAddButton={false}  field="GeoTag" header="GeoTag" sortable  headerStyle={{ minWidth: '10rem' }} filter filterPlaceholder="Search by GeoTag" ></Column>
    ...
    <Column showAddButton={false}  field="createAt" header="Created At" sortable  headerStyle={{ minWidth: '10rem' }} filterField="createAt" dataType="date" filter filterElement={createAtFilterTemplate}  ></Column>
                ...
                <Dialog visible={customerDialog} style={{ width: '450px' }} header="Customer Details" modal className="p-fluid" footer={customerDialogFooter} onHide={hideDialog}>
                    
                
    <div className="field">
        <label htmlFor="name">Name</label>
         <InputText id="name" value={customer.name} onChange={(e) => onInputChange(e, 'name')}    required className={classNames({ 'p-invalid': submitted && !customer.name })} />
    </div>
            

    <div className="field">
        <label htmlFor="email">Email</label>
         <InputText id="email" value={customer.email} onChange={(e) => onInputChange(e, 'email')}    />
    </div>
            

    <div className="field">
        <label htmlFor="Address">Address</label>
         <InputTextarea id="Address" value={customer.Address} onChange={(e) => onInputChange(e, 'Address')} rows={5} cols={30}    required className={classNames({ 'p-invalid': submitted && !customer.Address })} />
    </div>
            

    <div className="field">
        <label htmlFor="customerType">Customer Type</label>
         <Dropdown   id="customerType" optionLabel="name"  value={customer.customerType} options={datacustomerTypes} onChange={(e) => onInputChange(e, 'customerType')}  />
    </div>
            

    <div className="field">
        <label htmlFor="GeoTag">GeoTag</label>
         <InputText id="GeoTag" value={customer.GeoTag} onChange={(e) => onInputChange(e, 'GeoTag')}    required className={classNames({ 'p-invalid': submitted && !customer.GeoTag })} />
    </div>
            

    <div className="field">
        <label htmlFor="mobile">Mobile</label>
         <InputText id="mobile" value={customer.mobile} onChange={(e) => onInputChange(e, 'mobile')}    required className={classNames({ 'p-invalid': submitted && !customer.mobile })} />
    </div>
                ...
                <Dialog visible={deleteCustomersDialog} style={{ width: '450px' }} header="Confirm" modal footer={deleteCustomersDialogFooter} onHide={hideDeleteCustomersDialog}>
                    <div className="flex align-items-center justify-content-center">
                        <i className="pi pi-exclamation-triangle mr-3" style={{ fontSize: '2rem' }} />
                        {customer && <span>Are you sure you want to delete the selected Customer?</span>}
                    </div>
                </Dialog>
