...
import {MealGroup,MealGroupQuery,MealGroupKey, MealGroupService } from '@services/MealGroup';
        ...
        if (mealGroup.id) {
            ...
            let d=  await mealgroupService.updateMealGroup(_mealGroup);
                if(d.error==undefined){
                    ...
                    toast.current?.show({ severity: 'success', summary: 'Loaded', detail: 'MealGroup Updated', life: 3000 });
                }else{
...
const deleteMealGroup = async() => {
    ...
    if(d.error==undefined){
        ...
        setDeleteMealGroupDialog(false);
        setMealGroup(emptyMealGroup);
        toast.current?.show({ severity: 'warn', summary: 'Deleted', detail: 'MealGroup Deleted', life: 3000 });
    }else{
...
const deleteSelectedMealGroups = () => {
    let _mealGroups = mealGroups.filter((val) => !selectedMealGroups.includes(val));
    setMealGroups(_mealGroups);
    setDeleteMealGroupsDialog(false);
    setSelectedMealGroups([]);
    toast.current?.show({ severity: 'success', summary: 'Successful', detail: 'MealGroups Deleted', life: 3000 });
};
...
const localFilter=(val:string)=>{
    ...
    if(val.length>1){
        let _mealGroups =[...mealGroups];
        let filtered = _mealGroups.filter((data) =>  JSON.stringify(data).toLowerCase().indexOf(val.toLowerCase()) !== -1);
    ...
    return (
        <React.Fragment>
            <div className="my-2">
                ...
                <Button label="Delete" icon="pi pi-trash" className="p-button-danger" onClick={confirmDeleteSelected} disabled={!selectedMealGroups || !selectedMealGroups.length} />
            </div>
        </React.Fragment>
    );
};
...
const actionBodyTemplate = (rowData:MealGroup) => {
    return (
        <>
            <Button icon="pi pi-pencil" className="p-button-rounded p-button-success mr-2" onClick={() => editMealGroup(rowData)} />
            <Button icon="pi pi-trash" className="p-button-rounded p-button-warning" onClick={() => confirmDeleteMealGroup(rowData)} />
...
const mealGroupDialogFooter = (
    <>
        ...
        <Button label="Save" icon="pi pi-check" className="p-button-text" onClick={saveMealGroup} />
    </>
);
const deleteMealGroupDialogFooter = (
    <>
        <Button label="No" icon="pi pi-times" className="p-button-text" onClick={hideDeleteMealGroupDialog} />
        <Button label="Yes" icon="pi pi-check" className="p-button-text" onClick={deleteMealGroup} />
    </>
);
const deleteMealGroupsDialogFooter = (
    <>
        <Button label="No" icon="pi pi-times" className="p-button-text" onClick={hideDeleteMealGroupsDialog} />
        <Button label="Yes" icon="pi pi-check" className="p-button-text" onClick={deleteSelectedMealGroups} />
    </>
);
...
return (
        ...
        <div className="col-12">
            <div className="card">
                ...
                <DataTable
                    ...
                    onSelectionChange={(e) => setSelectedMealGroups(e.value as MealGroup[])}
                    dataKey="id"
                    loading={loading}
                    filters={filters1}
                    showGridlines
    ...
    <Column showAddButton={false}  field="updateBy" header="Update By" sortable  headerStyle={{ minWidth: '10rem' }} filter filterPlaceholder="Search by updateBy" ></Column>
                ...
                <Dialog visible={mealGroupDialog} style={{ width: '450px' }} header="MealGroup Details" modal className="p-fluid" footer={mealGroupDialogFooter} onHide={hideDialog}>
                    
                
    <div className="field">
        <label htmlFor="name">Name</label>
         <InputText id="name" value={mealGroup.name} onChange={(e) => onInputChange(e, 'name')}    required className={classNames({ 'p-invalid': submitted && !mealGroup.name })} />
    </div>
            

                </Dialog>

                <Dialog visible={deleteMealGroupDialog} style={{ width: '450px' }} header="Confirm" modal footer={deleteMealGroupDialogFooter} onHide={hideDeleteMealGroupDialog}>
                ...
                <Dialog visible={deleteMealGroupsDialog} style={{ width: '450px' }} header="Confirm" modal footer={deleteMealGroupsDialogFooter} onHide={hideDeleteMealGroupsDialog}>
                    <div className="flex align-items-center justify-content-center">
                        <i className="pi pi-exclamation-triangle mr-3" style={{ fontSize: '2rem' }} />
                        {mealGroup && <span>Are you sure you want to delete the selected MealGroup?</span>}
                    </div>
                </Dialog>
