...
import {MealItem,MealItemQuery,MealItemKey, MealItemService } from '@services/MealItem';
...
{id:'description',type:validate.text,max:200,min:0,required:true},
...
const initFilters1 = () => {
    setFilters1({
        ...
        groupName: { operator: FilterOperator.AND, constraints: [{ value: null, matchMode: FilterMatchMode.EQUALS }] },
...
weekdays: { operator: FilterOperator.AND, constraints: [{ value: null, matchMode: FilterMatchMode.IN }] },
active: { operator: FilterOperator.AND, constraints: [{ value: null, matchMode: FilterMatchMode.EQUALS }] },
mealType: { operator: FilterOperator.AND, constraints: [{ value: null, matchMode: FilterMatchMode.IN }] },
kitchen: { operator: FilterOperator.AND, constraints: [{ value: null, matchMode: FilterMatchMode.EQUALS }] }
    ...
    const updateFileName = (newUploadedFileName:string,colName:MealItemKey) => {
    ...
    const showUploadDialog = (mealItem:MealItem,dbColName:string,accept:string="images/*") => {
...
const priceFilterTemplate = (options:any) => {
    ...
    return (
            <>
                ...
                <Dropdown value={options.value} options={datamealGroups}  onChange={(e) => options.filterCallback(e.value)} optionLabel="name" optionValue="name" placeholder="Any" className="p-column-filter" />
            </>
        );
    }
    ...
    return (
        <>
            ...
            <MultiSelect value={options.value} options={dataweekdayss}  onChange={(e) => options.filterCallback(e.value)} optionLabel="Week Days" placeholder="Any" className="p-column-filter" />
        </>
        );
    }
...
const activeFilterTemplate = (options:any) => {
    ...
    return (
        <>
            ...
            <MultiSelect value={options.value} options={datamealTypes}  onChange={(e) => options.filterCallback(e.value)} optionLabel="Meal type" placeholder="Any" className="p-column-filter" />
        </>
        );
    }
    ...
    return (
            <>
                ...
                <Dropdown value={options.value} options={datakitchens}  onChange={(e) => options.filterCallback(e.value)} optionLabel="kitchenName" optionValue="id" placeholder="Any" className="p-column-filter" />
            </>
        );
    }
        ...
        if (mealItem.id) {
            ...
            let d=  await mealitemService.updateMealItem(_mealItem);
                if(d.error==undefined){
                    ...
                    toast.current?.show({ severity: 'success', summary: 'Loaded', detail: 'MealItem Updated', life: 3000 });
                }else{
...
const deleteMealItem = async() => {
    ...
    if(d.error==undefined){
        ...
        setDeleteMealItemDialog(false);
        setMealItem(emptyMealItem);
        toast.current?.show({ severity: 'warn', summary: 'Deleted', detail: 'MealItem Deleted', life: 3000 });
    }else{
...
const deleteSelectedMealItems = () => {
    let _mealItems = mealItems.filter((val) => !selectedMealItems.includes(val));
    setMealItems(_mealItems);
    setDeleteMealItemsDialog(false);
    setSelectedMealItems([]);
    toast.current?.show({ severity: 'success', summary: 'Successful', detail: 'MealItems Deleted', life: 3000 });
};
...
const localFilter=(val:string)=>{
    ...
    if(val.length>1){
        let _mealItems =[...mealItems];
        let filtered = _mealItems.filter((data) =>  JSON.stringify(data).toLowerCase().indexOf(val.toLowerCase()) !== -1);
    ...
    return (
        <React.Fragment>
            <div className="my-2">
                ...
                <Button label="Delete" icon="pi pi-trash" className="p-button-danger" onClick={confirmDeleteSelected} disabled={!selectedMealItems || !selectedMealItems.length} />
            </div>
        </React.Fragment>
    );
};
...
const actionBodyTemplate = (rowData:MealItem) => {
    return (
        <>
            <Button icon="pi pi-pencil" className="p-button-rounded p-button-success mr-2" onClick={() => editMealItem(rowData)} />
            <Button icon="pi pi-trash" className="p-button-rounded p-button-warning" onClick={() => confirmDeleteMealItem(rowData)} />
...
const mealItemDialogFooter = (
    <>
        ...
        <Button label="Save" icon="pi pi-check" className="p-button-text" onClick={saveMealItem} />
    </>
);
const deleteMealItemDialogFooter = (
    <>
        <Button label="No" icon="pi pi-times" className="p-button-text" onClick={hideDeleteMealItemDialog} />
        <Button label="Yes" icon="pi pi-check" className="p-button-text" onClick={deleteMealItem} />
    </>
);
const deleteMealItemsDialogFooter = (
    <>
        <Button label="No" icon="pi pi-times" className="p-button-text" onClick={hideDeleteMealItemsDialog} />
        <Button label="Yes" icon="pi pi-check" className="p-button-text" onClick={deleteSelectedMealItems} />
    </>
);
...
return (
        ...
        <div className="col-12">
            <div className="card">
                ...
                <DataTable
                    ...
                    onSelectionChange={(e) => setSelectedMealItems(e.value as MealItem[])}
                    dataKey="id"
                    loading={loading}
                    filters={filters1}
                    showGridlines
    ...
    <Column showAddButton={false}  field="groupName" header="Group Name" sortable  headerStyle={{ minWidth: '10rem' }} filterField="groupName"   filter filterElement={groupNameFilterTemplate}  ></Column>
    ...
    <Column showAddButton={false}  field="createAt" header="Created At" sortable  headerStyle={{ minWidth: '10rem' }} filterField="createAt" dataType="date" filter filterElement={createAtFilterTemplate}  ></Column>
                ...
                <Dialog visible={mealItemDialog} style={{ width: '450px' }} header="MealItem Details" modal className="p-fluid" footer={mealItemDialogFooter} onHide={hideDialog}>
                    
                
    <div className="field">
        <label htmlFor="groupName">Group Name</label>
         <Dropdown   id="groupName"  optionLabel="name" optionValue="name"  value={mealItem.groupName} options={datamealGroups} onChange={(e) => onInputChange(e, 'groupName')}  />
    </div>
            

    <div className="field">
        <label htmlFor="name">name</label>
         <InputText id="name" value={mealItem.name} onChange={(e) => onInputChange(e, 'name')}    required className={classNames({ 'p-invalid': submitted && !mealItem.name })} />
    </div>
            

    <div className="field">
        <label htmlFor="code">Code</label>
         <InputText id="code" value={mealItem.code} onChange={(e) => onInputChange(e, 'code')}    required className={classNames({ 'p-invalid': submitted && !mealItem.code })} />
    </div>
        ...
        {dataweekdayss.map((item) => {
            return (
                <div key={item.value} className="flex align-items-center">
                    <Checkbox inputId={item.value} name="weekdays" value={item} onChange={(e) => onInputChange(e, 'weekdays')}  checked={mealItem.weekdays.some((i:any) => i === item.value)} />
    ...
    <div className="field">
        ...
        {datamealTypes.map((item) => {
            return (
                <div key={item.value} className="flex align-items-center">
                    <Checkbox inputId={item.value} name="mealType" value={item} onChange={(e) => onInputChange(e, 'mealType')}  checked={mealItem.mealType.some((i:any) => i === item.value)} />
                    <label htmlFor={item.value} className="ml-2">{item.name}</label>
                </div>
            );
        })}
    ...
    <div className="field">
        <label htmlFor="description">description</label>
         <InputTextarea id="description" value={mealItem.description} onChange={(e) => onInputChange(e, 'description')} rows={5} cols={30}    required className={classNames({ 'p-invalid': submitted && !mealItem.description })} />
    </div>
            

    <div className="field">
        <label htmlFor="price">price</label>
         <InputNumber id="price" value={mealItem.price} onValueChange={(e) => onInputNumberChange(e, 'price')}  />
    </div>
    ...
    <div className="field">
        <label htmlFor="active">Active</label>
         <TriStateCheckbox  name="active"  id="active" value={mealItem.active} onChange={(e) => onInputBooleanChange(e, 'active')}  />
    </div>
            
                </Dialog>

                <Dialog visible={deleteMealItemDialog} style={{ width: '450px' }} header="Confirm" modal footer={deleteMealItemDialogFooter} onHide={hideDeleteMealItemDialog}>
                ...
                <Dialog visible={deleteMealItemsDialog} style={{ width: '450px' }} header="Confirm" modal footer={deleteMealItemsDialogFooter} onHide={hideDeleteMealItemsDialog}>
                    <div className="flex align-items-center justify-content-center">
                        <i className="pi pi-exclamation-triangle mr-3" style={{ fontSize: '2rem' }} />
                        {mealItem && <span>Are you sure you want to delete the selected MealItem?</span>}
                    </div>
                </Dialog>
    ...
    <Dialog visible={uploadDialog} style={{ width: '450px' }} header={`Upload ${uploadInfo?.dbColName}`} modal  onHide={hideUploadDialog}>
        <div className="flex align-items-center justify-content-center">
        <CustomFileUpload onUpload={(e)=>updateFileName(e,uploadInfo?.dbColName as keyof MealItem)} url={uploadInfo?.url} table="mealItem" tableId={mealItem.id } maxFileSize={1000000} accept={uploadInfo?.accept} fieldName="uploadFile" dbColName={uploadInfo?.dbColName} />
        </div>
    </Dialog>  
