...
import {MealPrepare,MealPrepareQuery,MealPrepareKey, MealPrepareService } from '@services/MealPrepare';
...
{id:'startTime',type:validate.datetime,max:0,min:0,required:true},
{id:'endTime',type:validate.datetime,max:0,min:0,required:true},
{id:'status',type:validate.text,required:true},
{id:'deliveryTime',type:validate.datetime,max:0,min:0,required:true}
    ]
...
const [deleteMealPreparesDialog, setDeleteMealPreparesDialog] = useState(false);
const [mealPrepare, setMealPrepare] = useState<MealPrepare>(emptyMealPrepare);
const [selectedMealPrepares, setSelectedMealPrepares] = useState<MealPrepare[]>([]);
...
startTime: { operator: FilterOperator.AND, constraints: [{ value: null, matchMode: FilterMatchMode.DATE_IS }] },
endTime: { operator: FilterOperator.AND, constraints: [{ value: null, matchMode: FilterMatchMode.DATE_IS }] },
status: { operator: FilterOperator.AND, constraints: [{ value: null, matchMode: FilterMatchMode.EQUALS }] },
deliveryTime: { operator: FilterOperator.AND, constraints: [{ value: null, matchMode: FilterMatchMode.DATE_IS }] },
        ...
        if (mealPrepare.id) {
            ...
            let d=  await mealprepareService.updateMealPrepare(_mealPrepare);
                if(d.error==undefined){
                    ...
                    toast.current?.show({ severity: 'success', summary: 'Loaded', detail: 'MealPrepare Updated', life: 3000 });
                }else{
...
const deleteMealPrepare = async() => {
    ...
    if(d.error==undefined){
        let _mealPrepares = mealPrepares.filter((val) => val.id !== mealPrepare.id);
        setMealPrepares(_mealPrepares);
        setBackupMealPrepares(_mealPrepares);
        setDeleteMealPrepareDialog(false);
        setMealPrepare(emptyMealPrepare);
        toast.current?.show({ severity: 'warn', summary: 'Deleted', detail: 'MealPrepare Deleted', life: 3000 });
    }else{
...
const deleteSelectedMealPrepares = () => {
    let _mealPrepares = mealPrepares.filter((val) => !selectedMealPrepares.includes(val));
    setMealPrepares(_mealPrepares);
    setDeleteMealPreparesDialog(false);
    setSelectedMealPrepares([]);
    toast.current?.show({ severity: 'success', summary: 'Successful', detail: 'MealPrepares Deleted', life: 3000 });
};
...
const localFilter=(val:string)=>{
    ...
    if(val.length>1){
        ...
        let filtered = _mealPrepares.filter((data) =>  JSON.stringify(data).toLowerCase().indexOf(val.toLowerCase()) !== -1);
    ...
    return (
        <React.Fragment>
            <div className="my-2">
                ...
                <Button label="Delete" icon="pi pi-trash" className="p-button-danger" onClick={confirmDeleteSelected} disabled={!selectedMealPrepares || !selectedMealPrepares.length} />
            </div>
        </React.Fragment>
    );
};
...
const actionBodyTemplate = (rowData:MealPrepare) => {
    return (
        <>
            <Button icon="pi pi-pencil" className="p-button-rounded p-button-success mr-2" onClick={() => editMealPrepare(rowData)} />
            <Button icon="pi pi-trash" className="p-button-rounded p-button-warning" onClick={() => confirmDeleteMealPrepare(rowData)} />
...
const mealPrepareDialogFooter = (
    <>
        ...
        <Button label="Save" icon="pi pi-check" className="p-button-text" onClick={saveMealPrepare} />
    </>
);
const deleteMealPrepareDialogFooter = (
    <>
        <Button label="No" icon="pi pi-times" className="p-button-text" onClick={hideDeleteMealPrepareDialog} />
        <Button label="Yes" icon="pi pi-check" className="p-button-text" onClick={deleteMealPrepare} />
    </>
);
const deleteMealPreparesDialogFooter = (
    <>
        <Button label="No" icon="pi pi-times" className="p-button-text" onClick={hideDeleteMealPreparesDialog} />
        <Button label="Yes" icon="pi pi-check" className="p-button-text" onClick={deleteSelectedMealPrepares} />
    </>
);
...
return (
        ...
        <div className="col-12">
            <div className="card">
                ...
                <DataTable
                    ...
                    onSelectionChange={(e) => setSelectedMealPrepares(e.value as MealPrepare[])}
                    dataKey="id"
                    loading={loading}
                    filters={filters1}
                    showGridlines
    ...
    <Column showAddButton={false}  field="startTime" header="startTime" sortable  headerStyle={{ minWidth: '10rem' }} filterField="startTime" dataType="date" filter filterElement={startTimeFilterTemplate}  ></Column>
            

    <Column showAddButton={false}  field="endTime" header="endTime" sortable  headerStyle={{ minWidth: '10rem' }} filterField="endTime" dataType="date" filter filterElement={endTimeFilterTemplate}  ></Column>
    ...
    <Column showAddButton={false}  field="deliveryTime" header="Delivery Time" sortable  headerStyle={{ minWidth: '10rem' }} filterField="deliveryTime" dataType="date" filter filterElement={deliveryTimeFilterTemplate}  ></Column>
    ...
    <Column showAddButton={false}  field="kitchen" header="Kitchen" sortable  headerStyle={{ minWidth: '10rem' }} filter filterPlaceholder="Search by kitchen" ></Column>
                ...
                <Dialog visible={mealPrepareDialog} style={{ width: '450px' }} header="MealPrepare Details" modal className="p-fluid" footer={mealPrepareDialogFooter} onHide={hideDialog}>
                    
                
    <div className="field">
        <label htmlFor="chefId">chefId</label>
         <AutoComplete field="name" id="chefId"  completeMethod={searchChef} value={mealPrepare.chefId} suggestions={sugchefs} onChange={(e) => onInputChange(e, 'chefId')}  />
    </div>
            

    <div className="field">
        <label htmlFor="orderId">orderId</label>
         <AutoComplete field="invoiceNo" id="orderId"  completeMethod={searchMealOrder} value={mealPrepare.orderId} suggestions={sugmealOrders} onChange={(e) => onInputChange(e, 'orderId')}  />
    </div>
            

    <div className="field">
        <label htmlFor="startTime">startTime</label>
         <Calendar id="startTime" value={mealPrepare.startTime?new Date(mealPrepare.startTime):null }  onChange={(e) => onInputChange(e, 'startTime')} showTime hourFormat="12"     required className={classNames({ 'p-invalid': submitted && !mealPrepare.startTime })} />
    </div>
            

    <div className="field">
        <label htmlFor="endTime">endTime</label>
         <Calendar id="endTime" value={mealPrepare.endTime?new Date(mealPrepare.endTime):null }  onChange={(e) => onInputChange(e, 'endTime')} showTime hourFormat="12"     required className={classNames({ 'p-invalid': submitted && !mealPrepare.endTime })} />
    </div>
            

    <div className="field">
        <label htmlFor="status">status</label>
         <Dropdown   id="status" optionLabel="name"  value={mealPrepare.status} options={datastatuss} onChange={(e) => onInputChange(e, 'status')}  />
    </div>
            

    <div className="field">
        <label htmlFor="deliveryTime">Delivery Time</label>
         <Calendar id="deliveryTime" value={mealPrepare.deliveryTime?new Date(mealPrepare.deliveryTime):null }  onChange={(e) => onInputChange(e, 'deliveryTime')} showTime hourFormat="12"     required className={classNames({ 'p-invalid': submitted && !mealPrepare.deliveryTime })} />
    </div>
            
                </Dialog>

                <Dialog visible={deleteMealPrepareDialog} style={{ width: '450px' }} header="Confirm" modal footer={deleteMealPrepareDialogFooter} onHide={hideDeleteMealPrepareDialog}>
                ...
                <Dialog visible={deleteMealPreparesDialog} style={{ width: '450px' }} header="Confirm" modal footer={deleteMealPreparesDialogFooter} onHide={hideDeleteMealPreparesDialog}>
                    <div className="flex align-items-center justify-content-center">
                        <i className="pi pi-exclamation-triangle mr-3" style={{ fontSize: '2rem' }} />
                        {mealPrepare && <span>Are you sure you want to delete the selected MealPrepare?</span>}
                    </div>
                </Dialog>
