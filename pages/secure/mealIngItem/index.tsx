...
const MealIngItemPage = () => {
  ...
  const [deleteMealIngItemsDialog, setDeleteMealIngItemsDialog] =
    useState(false);
  const [mealIngItem, setMealIngItem] = useState<MealIngItem>(emptyMealIngItem);
  const [selectedMealIngItems, setSelectedMealIngItems] = useState<
    MealIngItem[]
  >([]);
  ...
  const deleteMealIngItem = async () => {
    ...
    if (d.error == undefined) {
      let _mealIngItems = mealIngItems.filter(
        (val) => val.id !== mealIngItem.id
      );
      setMealIngItems(_mealIngItems);
      setBackupMealIngItems(_mealIngItems);
      setDeleteMealIngItemDialog(false);
  ...
  const deleteSelectedMealIngItems = () => {
    let _mealIngItems = mealIngItems.filter(
      (val) => !selectedMealIngItems.includes(val)
    );
    setMealIngItems(_mealIngItems);
    setDeleteMealIngItemsDialog(false);
  ...
  const localFilter = (val: string) => {
    if (val.length > 1) {
      ...
      let filtered = _mealIngItems.filter(
        (data) =>
      ...
      <React.Fragment>
        <div className="my-2">
          ...
          <Button
            ...
            disabled={!selectedMealIngItems || !selectedMealIngItems.length}
          />
        </div>
      </React.Fragment>
    );
    ...
    return (
      <>
        <Button
          ...
          onClick={() => editMealIngItem(rowData)}
        />
        <Button
          icon="pi pi-trash"
          className="p-button-rounded p-button-warning"
          onClick={() => confirmDeleteMealIngItem(rowData)}
        />
      ...
      <div className="col-12">
        <div className="card">
          ...
          <DataTable
            ...
            onSelectionChange={(e) =>
              setSelectedMealIngItems(e.value as MealIngItem[])
            }
            dataKey="id"
            loading={loading}
            filters={filters1}
          ...
          >
            ...
            <Column
              ...
              filter
              filterElement={ingredientsFilterTemplate}
            ></Column>
          ...
          <Dialog
            visible={deleteMealIngItemDialog}
            style={{ width: "450px" }}
            header="Confirm"
            modal
            footer={deleteMealIngItemDialogFooter}
            onHide={hideDeleteMealIngItemDialog}
          >
          ...
          <Dialog
            visible={deleteMealIngItemsDialog}
            style={{ width: "450px" }}
            header="Confirm"
            modal
            footer={deleteMealIngItemsDialogFooter}
            onHide={hideDeleteMealIngItemsDialog}
          >
            <div className="flex align-items-center justify-content-center">
              <i
                className="pi pi-exclamation-triangle mr-3"
              ...
              {mealIngItem && (
                <span>
                  Are you sure you want to delete the selected MealIngItem?
                </span>
              )}
            </div>
          </Dialog>
