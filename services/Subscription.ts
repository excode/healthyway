import {getDatas, postData,patchData,deleteData,getDataAll,getDataSuggestions,getData} from '@lib/httpRequest'
  ...
  export type SubscriptionQuery = Omit<Subscription, 'customerId'|'startDate'|'endDate'|'status'> & {
    customerId?: string;
startDate?: Date;
endDate?: Date;
status?: string
...
export class SubscriptionService {
  ...
  getSubscriptionAll(request:SubscriptionQuery) {
    return getDataAll<Subscription,SubscriptionQuery>( '/subscription/all',request) 
  }
  getSubscriptionSuggestions(keyword:string) {
    return getDataSuggestions<Subscription>( '/subscription/suggestions',keyword) 
  }
